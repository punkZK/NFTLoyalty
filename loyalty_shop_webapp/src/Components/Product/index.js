...
const Product = ({cardNFT, setCardNFT}) => {
    ...
    const buyProductAction = async () => {
        try {
            if(loyaltyContract) {
                ...
                const priceTxn = await loyaltyContract.getProductPriceInEth(0.001);
                console.log('The product cost is: {}', priceTxn);
                setStampState('purchasing');
                const purchaseTxn = await loyaltyContract.purchaseProduct(0.001, { "value": priceTxn});
    ...
    const stampLoyaltyCardNFTAction = async () => {
        try{
            if(loyaltyContract) {
                setRedeemState('stamping');
                console.log('Stamp loyalty card ...');
                const stampTxn = await loyaltyContract.stampLoyaltyCard();
                await stampTxn.wait();
                console.log('stampTxn:', stampTxn);
                setRedeemState('stamped');
            }

        } catch(error) {
            console.log("Error stamping card");
            setRedeemState('');
        }
    };
    ...
    const renderProducts = () => products.map( (product, index) => (
        <div className="card-item" key={product.name}>
            ...
            <img src={`https://cloudflare-ipfs.com/ipfs/${product.imageURI}`} alt={product.name}/>
            <button
            type="button"
    ...
    return(
        <div className="product-container">
            ...
            type="button"
            className="card-mint-button"
            onClick={()=> stampLoyaltyCardNFTAction()}
            >
            {`Stamp ${cardNFT.name}`}
            </button>
            <button
